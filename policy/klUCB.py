...
class klUCB(IndexPolicy):
    ...
    def computeIndex(self, arm):
        if self.nbDraws[arm] == 0:
            ...
            return self.klucb(self.cumReward[arm] / self.nbDraws[arm],
                              (log(self.t) + self.c * log(log(self.t))) / self.nbDraws[arm],
                              1e-4)  # Could adapt tolerance to the value of self.t
